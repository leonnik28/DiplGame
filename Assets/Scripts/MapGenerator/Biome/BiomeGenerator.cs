...
public class BiomeGenerator : MonoBehaviour
{
    ...
    public void GenerateBiomes()
    {
        ...
        for (int i = 0; i < _biomeCount; i++)
        {
            ...
            {
                // Новый биом размещается рядом с последним добавленным биомом
    ...
    private Vector3 FindBestPosition(Rect lastBiomeRect, int sizeX, int sizeY, List<Rect> biomeRects)
    {
        ...
        {
        new Vector3(lastBiomeRect.xMax, 0, lastBiomeRect.y), // Вправо
        new Vector3(lastBiomeRect.x, 0, lastBiomeRect.yMax), // Вверх
        new Vector3(lastBiomeRect.xMin - sizeX, 0, lastBiomeRect.y), // Влево
        new Vector3(lastBiomeRect.x, 0, lastBiomeRect.yMin - sizeY), // Вниз
        };
